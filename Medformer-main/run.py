...
if __name__ == "__main__":
    ...
    if args.task_name == "long_term_forecast":
        ...
        Exp = Exp_Classification    #Exp为Exp_Classification的引用
    else:
        Exp = Exp_Long_Term_Forecast
    ...
    if args.is_training:
        ...
        for ii in range(args.itr):
            ...
            exp.train(setting)  # 实验入口

            print(
                ">>>>>>>testing : {}<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<".format(setting)
            )
            accuracy_list.append(exp.test(setting))
            print()
            torch.cuda.empty_cache()
        print("Accuracies from each test:")
        for i, accuracy in enumerate(accuracy_list):
            print(f"Test {i + 1} Accuracy: {accuracy:.5f}")
