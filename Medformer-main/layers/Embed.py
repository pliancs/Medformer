...
class PositionalEmbedding(nn.Module):
    def __init__(self, d_model, max_len=5000):
        ...
        # 初始化一个零矩阵，禁止对 pe 的梯度计算，因为位置编码是固定的，不需要学习。
        pe = torch.zeros(max_len, d_model).float()
        pe.require_grad = False
        # unsqueeze方法用于在指定的维度上增加一个维度。将生成的张量转换为浮点数类型（float），然后由1维升到2维
        position = torch.arange(0, max_len).float().unsqueeze(1)
        # arange(0, d_model, 2)生成偶数索引，转化为浮点数，一个用于缩放的系数，使得位置编码的频率在维度上有所差异。
        div_term = (
        ...
        pe = pe.unsqueeze(0)
        # 将pe注册为模型的缓冲区，这样它不会被视为模型的参数（不会更新），但仍会被保存和加载。
...
class DataEmbedding(nn.Module):
    # c_in：输入特征的维度。
    # d_model：嵌入的输出维度，即高维空间的大小。
    # embed_type：嵌入类型，可以是"fixed"或其他类型，用于决定时间嵌入的方式。
    # freq：时间序列的频率，例如"h"代表小时，可以决定时间嵌入的粒度。
    # dropout：Dropout的概率，用于正则化。
    def __init__(self, c_in, d_model, embed_type="fixed", freq="h", dropout=0.1):
        super(DataEmbedding, self).__init__()
        # 值嵌入，从c维度转为为更高的维度d维度，涉及到了环绕填充以及卷积
        self.value_embedding = TokenEmbedding(c_in=c_in, d_model=d_model)
        # 位置嵌入，位置嵌入提供序列中每个位置的唯一表示，使模型能够捕捉时序依赖关系。
        self.position_embedding = PositionalEmbedding(d_model=d_model)
        # 时间嵌入，这里根据 embed_type 和 freq 选择时间嵌入方式。如果 embed_type 不是 "timeF"，则使用 TemporalEmbedding；
        # 否则使用 TimeFeatureEmbedding。TemporalEmbedding 和 TimeFeatureEmbedding 通过时间特征向量（如时钟时间、日期等）增强模型对时间特征的理解。
