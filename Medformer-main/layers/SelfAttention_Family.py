...
class FullAttention(nn.Module): #全局自注意力机制（FullAttention），带有掩码选项，用于防止未来信息泄露。
    def __init__(
        self,
        mask_flag=True, #否使用掩码，如果为True，模型会避免使用未来信息。
        factor=5,   #通常用于注意力计算的加速因子，但在此实现中未直接使用。
        scale=None, #缩放因子，用于对注意力得分进行缩放。如果为空，则使用默认的 1 / sqrt(E)。
        attention_dropout=0.1,  #对注意力得分应用的 dropout，以增强泛化能力。
        output_attention=False, #控制是否输出注意力权重 A。如果 True，返回注意力权重以供进一步分析。
    ):
    ...
    def forward(self, queries, keys, values, attn_mask, tau=None, delta=None):
        #queries, keys, values：表示输入的查询、键和值。它们形状为 [B, L, H, E] 和 [B, S, H, D]，其中：
        # B：批大小。
        # L和S：序列长度。
        # H：注意力头数。
        # E和D：每个头的嵌入维度。
        ...
        # 使用爱因斯坦求和表示法torch.einsum计算注意力得分scores，其维度为[B, H, L, S]。scores[i][j]
        # 表示第i个批次、第j个注意力头中查询和键的相似度，确定序列中各位置的相对重要性
        scores = torch.einsum("blhe,bshe->bhls", queries, keys)

        if self.mask_flag:  #若 mask_flag=True，且未指定掩码 attn_mask，则生成 TriangularCausalMask 掩码矩阵，确保模型仅访问过去的信息，避免未来信息泄露。
            if attn_mask is None:
                attn_mask = TriangularCausalMask(B, L, device=queries.device)   #masked_fill_ 会将掩码中为 True 的位置填充为负无穷，使得该位置的注意力得分在 softmax 后接近 0。

            scores.masked_fill_(attn_mask.mask, -np.inf)

        A = self.dropout(   #对缩放后的 scores 应用 softmax，得到注意力权重 A。
            torch.softmax(scale * scores, dim=-1)
        )  # Scaled Dot-Product Attention
        V = torch.einsum("bhls,bshd->blhd", A, values)  #使用 torch.einsum 将注意力权重 A 与 values 相乘，生成注意力输出 V，其维度为 [B, L, H, D]。

        if self.output_attention:   #若 output_attention=True，则返回注意力权重 A，便于后续的解释性分析
            return V.contiguous(), A
        else:
            return V.contiguous(), None #否则返回注意力输出 V 和 None。
