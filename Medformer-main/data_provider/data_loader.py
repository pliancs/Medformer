...
class APAVALoader(Dataset):
    def __init__(self, args, root_path, flag=None):
        self.root_path = root_path  #数据的根目录路径。
        self.data_path = os.path.join(root_path, "Feature/")    #数据
        self.label_path = os.path.join(root_path, "Label/label.npy")    #标签

        data_list = np.load(self.label_path)    #加载标签数据，获取所有样本的 ID。
    ...
    def load_apava(self, data_path, label_path, flag=None):
        ...
        Returns:
            ...
            y: (num_samples, ) np.array of labels
        """
        feature_list = []   #加载的特征和标签。
        label_list = []
        filenames = []  #存储数据文件名。
        # The first column is the label; the second column is the patient ID
        subject_label = np.load(label_path) #从标签文件中加载的标签数据。
        for filename in os.listdir(data_path):
            filenames.append(filename)
        filenames = natsorted(filenames)
        if flag == "TRAIN":
...
class ASDLoader(Dataset):
    def __init__(self, args, root_path, flag=None):
        ...
        all_ids = list(data_list[:, 1])  # id of all samples
        # val_ids = list(range(23, 31)) + list(range(61, 67))   # 15, 19 are AD; 16, 20 are HC
        val_ids = list(range(31, 39)) + list(range(67, 76))  # 15, 19 are AD; 16, 20 are HC
        test_ids = list(range(31, 39)) + list(range(67, 76))   # 1, 17 are AD; 2, 18 are HC
        train_ids = [int(i) for i in all_ids if i not in val_ids + test_ids]
        # list of IDs for training, val, and test sets
        self.train_ids, self.val_ids, self.test_ids = train_ids, val_ids, test_ids

        self.X, self.y = self.load_ASD(self.data_path, self.label_path, flag=flag)
    ...
    def load_ASD(self, data_path, label_path, flag=None):
        """
        Loads APAVA data from npy files in data_path based on flag and ids in label_path
        Args:
            data_path: directory of data files
